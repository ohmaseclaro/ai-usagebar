...
must_haves:
  truths:
    ...
    - "Chunking the same bytes twice produces the same chunk ids in the same order."
  ...
  artifacts:
    - src/sync/chunk.rs with the frame layout, the zstd stage, and the seal/reassemble pipeline
  key_links:
    - "chunk.rs calls Keys::chunk_id and Keys::seal — it never constructs a cipher itself"
    - "The frame's u32 true_len is what lets reassembly recover the exact tail length after padding"
---

<objective>
The chunker: fixed 256 KiB offset-aligned chunks plus an explicit tail, each framed with its true
length, compressed with zstd, padded to hide its exact size, and sealed through `crypto.rs`.
...
<task type="auto" tdd="true">
  <name>Task 1: The chunk frame — length prefix, zstd, padding, seal</name>
  ...
  <behavior>
    ...
    - Two seals of the same input produce byte-identical ciphertext and the same id.
...
Define the frame — the exact bytes handed to `Keys::seal` — as, in order: `u32` little-endian
`true_len` (the uncompressed byte count), `u32` little-endian `comp_len` (the exact length of the
zstd frame that follows), the zstd level-3 frame itself, then zero padding up to the next power of
two of `8 + comp_len`, capped at `CHUNK_SIZE`. Record this layout in the module doc comment; 1-08
copies it into `docs/sync-format.md`.
...
length. And every step is deterministic, so the same input yields the same frame, the same id, and
the same ciphertext, which is the precondition for dedup existing at all.
...
`pub fn seal_chunk(keys: &Keys, data: &[u8]) -> Result<Blob>` frames the data, takes
`keys.chunk_id(&frame)` as the id, and seals the frame under that id. The id is computed over the
frame rather than the raw input so that the recheck inside `Keys::open` compares like with like;
note in a comment that this ties chunk ids to the zstd version, which is exactly why
`FORMAT_VERSION` and `CHUNKER_ID` are recorded in every snapshot.

`pub fn open_chunk(keys: &Keys, id: &ChunkId, ciphertext: &[u8]) -> Result<Zeroizing<Vec<u8>>>` opens
through `crypto.rs` and unframes. Return the plaintext inside `Zeroizing` — it is a fragment of a
...
parameters `{ m_kib: 8, t: 1, p: 1 }`.
  ...
  <done>Every `<behavior>` line passes. The frame layout is documented in the module doc comment.</done>
</task>
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Reassembly given the chunks in the wrong order produces different bytes than the original — order is data, not a hint.
    - Reassembly with one chunk's ciphertext replaced by another chunk's errors rather than returning short or wrong bytes.
  </behavior>
  <action>
...
it must compare the actual id lists before and after, not merely the counts.
  </action>
  <verify>
    <automated>cargo test --lib sync::chunk</automated>
  </verify>
  <done>All `<behavior>` lines pass, including the append-stability assertion over real id lists.</done>
</task>
...
| T-02-03 | Tampering | `reassemble` | high | mitigate | Any chunk failure aborts the whole reassembly; no partial buffer is ever returned |
| T-02-04 | Information disclosure | total object count and sizes | low | accept | Hiding aggregate volume needs constant-rate cover traffic; documented as accepted leakage in 1-08 |
</threat_model>
...
2. Sealing the same bytes twice yields identical ciphertext.
3. An append leaves every previously sealed chunk id unchanged, asserted over the id lists.
4. A swapped or malformed chunk aborts reassembly with zero bytes returned.
</success_criteria>

<output>
Create `.planning/phases/1-encrypted-bundle-core/1-02-SUMMARY.md` when done. Record the exact frame
layout — 1-07 pins a known-answer vector against it and 1-08 documents it.
</output>
