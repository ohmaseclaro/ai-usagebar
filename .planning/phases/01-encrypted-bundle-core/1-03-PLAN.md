...
must_haves:
  truths:
    ...
    - "A pack's name is derived from its own bytes, so a substituted pack is detectable by name alone."
    - "The pack header carries no filename, path, or plaintext size in the clear."
    - "A pack truncated by a single byte fails to open rather than yielding the blobs that survived."
  artifacts:
    - src/sync/pack.rs with PackWriter, the encrypted trailing header, and the sharded name function
  key_links:
    - "The u32 LE header length is the last four bytes — it is what makes truncation immediately detectable"
    - "pack.rs calls crypto::content_address rather than importing a hash crate, preserving the containment invariant"
---

<objective>
The pack format: blob ciphertexts concatenated, then an encrypted header listing them, then a `u32`
little-endian header length. This is restic's layout, simplified, and it is what makes bulk upload
possible at all — GitHub's content-creation limits of 80 per minute and 500 per hour make one
request per chunk structurally impossible, so 5,000 chunks must become a handful of objects.
...
<task type="auto" tdd="true">
  <name>Task 1: PackWriter and the encrypted trailing header</name>
  ...
  <behavior>
    ...
    - The finished pack's last four bytes decode as a u32 LE header length that lands exactly at the header's start.
...
Layout, documented in the module doc comment: `<blob0 ciphertext><blob1 ciphertext>…<blobN
ciphertext><sealed header><u32 LE header length>`. The header is sealed exactly like a chunk — it is
serialized, then handed to `Keys::seal` under its own `crypto::content_address` as the id, so the
same associated-data binding that protects a chunk protects the header. Everything after the blob
region is therefore opaque; a reader that has the pack but not the keys learns the total size and
nothing else.
...
Vec<PackEntry> }` — serialize as JSON, zstd it before sealing (a few thousand entries of hex ids
compress hard), and refuse an unrecognised `format` on read before doing anything else, per **D-02
(D2)**.
...
`pub fn finish(self, keys: &Keys) -> Result<(ChunkId, Vec<u8>)>` returning the pack's content address
and its bytes. `finish` on an empty writer is an error.

`pub fn read_header(keys: &Keys, pack: &[u8]) -> Result<PackHeader>` — read the trailing `u32`, check
that the implied header region lies inside the buffer, open it, then check that every entry's
`offset + clen` also lies inside the blob region. Both checks happen before any entry is returned.
A truncated pack must fail here, not later at an out-of-bounds slice.
...
Write the `<behavior>` assertions inline with the cheap KDF parameters.
  ...
  <done>Every `<behavior>` line passes. A truncated or bit-flipped pack fails at `read_header` with no entry returned.</done>
</task>
...
Write the `<behavior>` assertions.
  ...
  <done>All `<behavior>` lines pass. Pack names are a pure function of pack bytes.</done>
</task>
...
|---|---|
| remote pack bytes → `read_header` | Every offset and length in a fetched pack is attacker-controlled until validated |
...
|---|---|---|---|---|---|
| T-03-01 | Tampering | `read_header` | critical | mitigate | Trailing `u32` and every entry's `offset + clen` are bounds-checked against the buffer before any entry is returned; the header is itself sealed, so truncation breaks its tag |
...
| T-03-03 | Information disclosure | pack header contents | high | mitigate | The header is sealed and carries no path, filename, or directory structure — only ids, offsets, and lengths |
| T-03-04 | Denial of service | crafted header length | high | mitigate | A header length larger than the pack is rejected before allocation |
</threat_model>
...
<success_criteria>
1. Multiple blobs pack and unpack byte-exactly through the encrypted header.
2. A single flipped bit anywhere in the header, and a single truncated byte, both fail to open.
3. No blob plaintext appears anywhere in a finished pack.
4. Pack names are content-addressed and sharded two levels deep.
</success_criteria>

<output>
Create `.planning/phases/1-encrypted-bundle-core/1-03-SUMMARY.md` when done. Record the exact pack
layout and the `PackEntry` fields — 1-06 truncates a real pack and 1-08 documents the format.
</output>
