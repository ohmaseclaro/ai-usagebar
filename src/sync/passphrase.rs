...
mod tests {
    ...
    fn no_password_input_path_reads_the_process_environment() {
        ...
        // The exemption names `src/sync/github/token.rs`, which arrives with
        // the transport PR; until then there is nothing to exempt. See the
        // note in `crypto.rs`'s sibling guard about the rising floor.
        assert_eq!(skipped, 0, "no exempt file exists on this branch yet");
        assert!(scanned >= 6, "only {scanned} files walked under src/sync");
    }
}
